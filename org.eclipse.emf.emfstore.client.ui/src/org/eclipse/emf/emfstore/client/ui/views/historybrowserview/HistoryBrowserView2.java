...
public class HistoryBrowserView2 extends ViewPart implements ProjectSpaceContainer {
	...
	private void addBaseVersionTag(List<HistoryInfo> infos) {
		...
		if (historyInfo != null) {
			...
			tagVersionSpec.setName(VersionSpec.BASE);
			historyInfo.getTagSpecs().add(tagVersionSpec);
		}
	}
			...
			return new ServerCall<List<HistoryInfo>>((ProjectSpaceBase) projectSpace) {
				...
				protected List<HistoryInfo> run() throws EmfStoreException {
					...
					if (projectSpace != modelElement) {
						...
						historyInfos.addAll(projectSpace.getHistoryInfo(HistoryQueryBuilder.rangeQuery(centerVersion,
	...
	private void initProjectDeleteListener() {
		WorkspaceManager.getObserverBus().register(new DeleteProjectSpaceObserver() {
			public void projectSpaceDeleted(ProjectSpace projectSpace) {
				if (HistoryBrowserView2.this.projectSpace == projectSpace) {
					setInput(null);
				}
			}
		});
	...
	private PrimaryVersionSpec prevNextCenter(boolean next) {
		...
		// if center is not on the selected branch (base versiob of ps) jump to base before paging
		...
		Action jumpTo = new Action() {
			...
			public void run() {
				...
				if (inputDialog.open() == Window.OK) {
					try {
						if (projectSpace != null) {
							PrimaryVersionSpec versionSpec = projectSpace.resolveVersionSpec(Versions.createPRIMARY(
								VersionSpec.GLOBAL, Integer.parseInt(inputDialog.getValue())));
							if (versionSpec != null) {
								showAll(true);
